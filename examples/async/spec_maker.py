...
MOVE_ACTIONS = [("r1","r2"),("r2","r3"),("r1","r3"),("r3","r2"),("r3","r1"),("r4","r3"),("r4","r5"),("r5","r4"),("r7","r5"),("r5","r7"),("r3","r4")]
NOF_ROBOTS = 3
...
AUX_PROPS_STAY_THE_SAME = [("req1","req1<->req1'"),("req2","req2<->req2'"),("req3","req3<->req3'"),("req4","req4<->req4'"),("req5","req5<->req5'")]
AUX_INIT = ["!req1 & !req2 & !req3 & !req4 & !req5"]
AUX_TRANS = [(["req1"],["req1' <-> ! req1"]),(["req2"],["req2' <-> ! req2"]),(["req3"],["req3' <-> ! req3"]),(["req4"],["req4' <-> ! req4"]),(["req5"],["req5' <-> ! req5"])]
AUX_ENV_LIVENESS = []
AUX_SYS_LIVENESS = ["r7_ready = "+str(NOF_ROBOTS)+" & completedTask_r3 & completedTask_r4 | req1 | req2 | req3 | req4 | req5","! req1 | r1_ready>0","! req2 | r2_ready>0","! req3 | r3_ready>0","! req4 | r4_ready>0","! req5 | r5_ready>0"]
...
for (rfrom,rto) in MOVE_ACTIONS:
    ...
    for (a,b) in AUX_PROPS_STAY_THE_SAME:
        print b
    print "opsuccess'"
    print "environmentDidAction'"
    print "_endgroup\n"
